...
--model  CONST poisson   apply a Poisson process for modelling indel/mutations, gets dosage distribution as parameter\n\
               ...
               scancat   apply a scan catalyze approach for modelling indel/mutations (default), acts independently on each homologue\n\
...
--empirical     STR  the name of the text file contaning numerical values corresponding to the distances between successive variants,\n\
                     ...
                     with --model option!\n\
--dosage LIST  percentage of simplex, duplex, ..., ploidy-plex variants among all the variants, used only with poisson and distance models.\n\
...
2) Double quotes should always be put around the lists and dictionaries specified on the command line.\n\
...
9) Dosage percentages must be given in a list, for simplex, duplex, ... up to the dosage equal to the ploidy level, respectively.'
		if not (opts or args):
			...
			raise getopt. GetoptError('ERROR: Invalid input format!' , None )
		else:
			pass
		for opt, arg in opts:
			if opt in ("-h", "--help"):
...
Missing probabilities will be set equally, so that the total sum of dosage probabilities becomes one!" %len(dosage)
 ...
 Missing probabilities will be set to zero!" %len(dosage)
				...
				if model==1 or model==2:
					_p=max(0.05,sum(rates)/float(sum(1 for x in rates if x>0)))  # Mean change rate for indel/mutation rates which are not zero, is used in the binomial distribution
				else:															 # to determine dosage percentages. A minimum of 0.05 is, however, considered for this probability.
					_p=sum(exp(y) for y in rates if y>0)/sum(1 for x in rates if x>0)
					_p=max(0.05, 1./_p)
