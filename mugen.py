...
Probabilities are set to zero for missing letters! Invalid letters are ignored!")
			...
			if nodel==0:
				...
				self.deleteprob=copy.deepcopy(new_deleteprob)	# Update deleteprob
			if nomu==0:
				pass
			elif nomu==1:
			...
			if noinsert==0:
				...
				self.insertprob=copy.deepcopy(new_insertprob)	# Update insertprob
		except CustomException as instance:
			print(instance)
	...
	def probmu(self):
		...
		for __site, __base in enumerate(self.ref):
			if __site in set(self.mupos)|set(self.inpos)|set(self.delpos):
				self.seq+=__base								 # No change is made at indel/mutation positions
			else:
				__prob={'ins':self.insertprob.get(__base),'del':self.deleteprob.get(__base),
					'sub':self.muprob.get(__base)}
				__error=random.choice(['ins','del','sub','sub']) # An error occurs randomly: insertion or \ 
																 # deletion or substitution
				__rnd=float(int(random.random()*100000))/100000  # The probability that this error is \
																 # not corrected by replication machinary is determined \			 	
				if __rnd<__prob.get(__error):					 # by insertprob,deleteprob and muprob
					if __error=='sub':
						self.seq+=random.choice(self.mualphabet.get(__base))	 # Substitute tha letter with one from the mutation alphabet 
						...
						self.seq+=__base
						self.seq+=random.choice(list(self.alphabet))			 # Insert a random letter right after the letter
						...
						self.occureddel.append(__site)			 # Delete the letter in the progeny sequence by just not adding it
				else:											 # Update the list of the sites which are deleted in the progeny sequence
					self.seq+=__base						     # No change is induced at the site in the progeny sequence	
		self.seq=MutableSeq(self.seq,self.alphaproperty)
		if (self.occuredins):
			_ins_allele=zip(self.occuredins,self.inserted_allele)
			_ins_allele.sort(key=lambda tup: tup[0])			 # Sort the occured change positions in ascending order
	...
	def posmu(self):
		...
		for __site, __error in iter(zip(range(len(self.ref)),__change)):
			...
			if __error is None:
				...
				self.occureddel.append(__site)					 # Delete the letter in the progeny sequence by just not adding it		
		self.seq=MutableSeq(self.seq,self.alphaproperty)		 # Update the list of the sites which are deleted in the progeny sequence	
		if (self.occuredins):
			_ins_allele=zip(self.occuredins,self.inserted_allele)
			_ins_allele.sort(key=lambda tup: tup[0])			 # Sort the occured change positions
			self.occuredins, self.inserted_allele= zip(*_ins_allele)
			self.occuredins=list(self.occuredins)
			self.inserted_allele=list(self.inserted_allele)
			_ins_allele=None
...
1)Mutation  2)Deletion 3)Insertion. \n")
	...
	def hapchanger(self):
		...
		self.alt_allele=list()
		for __site, __base in enumerate(self.ref):
			if __site in set(self.mupos): 						    # Making specified changes at the specified positions
				self.seq+=random.choice(self.mualphabet.get(__base))# Induce mutation at the site whose position is given
				self.occuredmu.append(__site)					    # Update the list of the sites where a mutation has occured
				self.alt_allele.extend([self.seq[-1]])	 		    # Update the list of alternative alleles
			elif __site in set(self.inpos):
				self.seq+=__base								    # Make an insertion right after the site whose position is given
				self.seq+=random.choice(list(self.alphabet))
				self.occuredins.append(__site)					    # Update the list of the sites after which an insertion has occured
				self.inserted_allele.extend([__base+self.seq[-1]])	# Update the list of inserted alleles
				...
				if __rnd<__prob.get(__error):					 # by insertprob,deleteprob and muprob
					if __error=='sub':
						self.seq+=random.choice(self.mualphabet.get(__base))
						self.occuredmu.append(__site)			 # Update the list of the sites where a mutation has occured
						self.alt_allele.extend([self.seq[-1]])	 # Update the list of alternative alleles
					elif __error=='ins':
						self.seq+=__base
